...
_PATRONES_BUSQUEDA = [
    "Fase4_sistema integral orientado a objetos_PaolaSalazar.py",
    "Fase4_sistema integral orientado a objetos_PaolaSalazar.py",
    "Fase4*sistema integral orientado a objetos*.py",
    "*Fase4*PaolaSalazar*.py",
    "sistema_base.py",
]
...
class VentanaSistemaGestion:
    ...
    def _construir_widgets(self) -> None:
        ...
        # Barra de progreso para la simulación
        self.barra_progreso = ttk.Progressbar(
            marco_acciones, 
            orient="horizontal", 
            length=200, 
            mode="determinate",
            maximum=10  # 10 operaciones
        )
        self.barra_progreso.pack(side="left", padx=15, pady=8)

        # Etiqueta de progreso (ej: "1/10")
        self.etiqueta_progreso = ttk.Label(marco_acciones, text="0/10")
        self.etiqueta_progreso.pack(side="left", padx=5, pady=8)
    ...
    def _on_ejecutar_simulacion(self) -> None:
        ...
        self.etiqueta_progreso.config(text="0/10")
    ...
    def _ejecutar_simulacion_en_hilo(self) -> None:
        ...
        try:
            ...
            self.cola_mensajes.put(("__PROGRESO_SIMULACION__", 0))
    ...
    def _simular_diez_operaciones(self) -> None:
        ...
        # Contador de operaciones para la barra de progreso
        operacion_actual = 0
        def reportar_progreso():
            """Función interna para actualizar el progreso."""
            nonlocal operacion_actual
            operacion_actual += 1
            self.cola_mensajes.put(("__PROGRESO_SIMULACION__", operacion_actual))
    ...
    def _procesar_cola_mensajes(self) -> None:
        ...
        try:
            while True:
                ...
                if isinstance(mensaje, tuple) and mensaje[0] == "__PROGRESO_SIMULACION__":
                    progreso = mensaje[1]
                    self.barra_progreso['value'] = progreso
                    self.etiqueta_progreso.config(text=f"{progreso}/10")
                    continue
