...
class GestorSistema:
    ...
    # REPORTES
    # ------------------------------------------------------------------
    def obtener_resumen_clientes(self) -> list[str]:
        """
        Retorna una lista con los resúmenes de todos los clientes registrados.
        
        Returns:
            list[str]: Lista de resúmenes de clientes. Si no hay clientes,
            retorna una lista con un mensaje informativo.
        """
        if not self.clientes:
            return ["(No hay clientes registrados todavía)."]
        return [f"  • {cliente.resumen()}" for cliente in self.clientes.values()]

    def listar_clientes(self) -> None:
        """
        Método de compatibilidad que imprime los resúmenes.
        Delega en obtener_resumen_clientes() y muestra el resultado.
        """
        for linea in self.obtener_resumen_clientes():
            print(linea)
                
    def obtener_resumen_servicios(self) -> list[str]:
        if not self.servicios:
            return ["(No hay servicios registrados todavía)."]
        return [f"  • {servicio.resumen()}" for servicio in self.servicios.values()]

    def listar_servicios(self) -> None:
        for linea in self.obtener_resumen_servicios():
            print(linea)

    def obtener_resumen_reservas(self) -> list[str]:
        if not self.reservas:
            return ["(No hay reservas registradas todavía)."]
        return [f"  • {reserva.resumen()}" for reserva in self.reservas.values()]

    def listar_reservas(self) -> None:
        for linea in self.obtener_resumen_reservas():
            print(linea)
